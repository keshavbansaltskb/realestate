...
export default function ChangePass(){
    ...
    useEffect(()=>{
        if(hasCookie("user") == false){
            ...
            axios.get("http://localhost:3000/api/real/password/"+id).then((res)=>setRec(res.data.result));
        }
    },[])
    function change(e){
        setMsg("");
    ...
    function update(ids){
        axios.put("http://localhost:3000/api/real/password",{cpass:ppass.cpass,npass:ppass.npass,rpass:ppass.rpass,id:ids}).then((res)=>setMsg(res.data.result));   
